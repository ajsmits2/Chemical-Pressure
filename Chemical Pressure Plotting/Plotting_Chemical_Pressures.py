...
#Bond drawing controls 
#To Remove Bonds quickly, set BondMinDist=0.01, and BondMaxDist=0.02
BondMinDist=0.1  #Minimum distance for two atoms to be considered bonded
BondMaxDist=3.0  #Maximum distance for two atoms to be considered bonded
BondRadius=0.05  #Radius of the bond cylinders
...
#The below code plots the atom bonds

#Matches the color scheme used above when drawing the atom spheres
def atom_color(atom_index):
    if atom_index == 0:
        return 'blue'
    elif atom_index == 1:
        return 'red'
        ...
        return 'orange'
    else:  #If more than 4 atoms are needed, expand this to match the sphere coloring section above
        return 'green'

bond_data = []  #Needed for scaling

def drawbonds_cylinder(xyz, numberofatoms, atom1, color1, atom2, color2, d_min, d_max, radius):
    natoms = len(xyz)
    new_cylinders = []
    for j in range(natoms):
        for k in range(natoms):
            if numberofatoms[j] != atom1 or numberofatoms[k] != atom2:
                continue

            xyz_j = np.array(xyz[j])
            xyz_k = np.array(xyz[k])
            dist_vec = xyz_k - xyz_j
            ...
            if atom1 == atom2:
                #Same element on both ends -> one full bond in a single color
                Cylinder = pv.Cylinder(center=xyz_j + dist_vec * 0.5, direction=dist_vec,
                                        radius=radius, height=dist, resolution=20)
                orig_pts = Cylinder.points.copy()
                center = xyz_j + dist_vec * 0.5
                plotter.add_mesh(Cylinder, style='surface', color=color1)
                bond_data.append((Cylinder, orig_pts, center))
                new_cylinders.append(Cylinder)
            else:
                #Different elements -> split into two halves, one colored per atom
                center1 = xyz_j + dist_vec * 0.25
                Cylinder1 = pv.Cylinder(center=center1, direction=dist_vec,
                                         radius=radius, height=dist * 0.5, resolution=20)
                orig_pts1 = Cylinder1.points.copy()
                plotter.add_mesh(Cylinder1, style='surface', color=color1)
                bond_data.append((Cylinder1, orig_pts1, center1))
                new_cylinders.append(Cylinder1)

                center2 = xyz_j + dist_vec * 0.75
                Cylinder2 = pv.Cylinder(center=center2, direction=dist_vec,
                                         radius=radius, height=dist * 0.5, resolution=20)
                orig_pts2 = Cylinder2.points.copy()
                plotter.add_mesh(Cylinder2, style='surface', color=color2)
                bond_data.append((Cylinder2, orig_pts2, center2))
                new_cylinders.append(Cylinder2)
    return new_cylinders

#Builds every unique element-pair combination
element_ids = sorted(set(numberofatoms))
bond_pairs = list(itertools.combinations_with_replacement(element_ids, 2))

all_bonds = []
for atom1, atom2 in bond_pairs:
    all_bonds.extend(drawbonds_cylinder(xyz, numberofatoms, atom1, atom_color(atom1),
                                         atom2, atom_color(atom2),
                                         BondMinDist, BondMaxDist, BondRadius))
